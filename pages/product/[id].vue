  ...
  <div class="product__content">
    ...
    <article class="product__info">
      <header class="product__img">
        <base-carousel class="carousel-product-card" v-if="productData.imgUrls.length >= 2" :img="productData.imgUrls" :type-product="true"></base-carousel>
      ...
      <article class="product__text-info block-style block-padding">
        ...
        <footer class="product__price-rating">
          <div class="price">
            <h2 class="text-subtitle--bold-style">{{ productData.price }} ₽</h2>
            <h3 class="text-subtitle-style text-color-gray" v-if="productData.originalPrice">{{ productData.originalPrice }} ₽</h3>
          </div>
          <div class="rating">
            <h2 class="text-subtitle--bold-style">{{ productData.rating }}</h2>
    ...
    <footer class="product__footer">
      <div class="product__foter--reviews block-style">
        <header-block-forward-arrow name="Отзывы" :link-forward="`/categories`">
          <template #amount>
            <h2 class="text-subtitle--bold-style amount-text-style">{{ productData.reviews.length }}</h2>
          </template>
        </header-block-forward-arrow>
        <ul class="product__reviews">
          <review-item-review v-for="(review, index) in productData.reviews" :key="review.id" :review="review" :last="index === productData.reviews.length - 1"></review-item-review>
        </ul>
      </div>
